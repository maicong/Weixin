...
class Wechat
{
	...
	*/
	...
	/**
	 * 设置回复彩票信息
	 * Examle: $obj->lottery('ssq')->reply();
	 * @param string $text
	 */
	public function lottery($text = '', $type = '')
	{

		if ($type === 'rand') {
			$info = $this->http_get('http://[************]/rand.php?key=' . $text);
		} elseif ($type === 'news') {

			if ($text === 'szcnews') {
				$url = 'http://[*********]/api_info.php?tid=[***]&num=5&type=json';
			} elseif ($text === 'zcnews') {
				$url = 'http://[*********]/api_info.php?tid=[***]&num=5&type=json';
			}
			...
			foreach($json as $key=>$val){
				$url = 'http://[************]/news.php?key='.$val['id'];
				array_push($info, array("Title" => $val['title'],"Description" => iconv_substr($val['description'], 0, 30),"PicUrl" => $val['picurl'],"Url" => $url));
			}
			return $this->news($info);
			exit();
		} else {
			$info = $this->http_get('http://[************]/lottery.php?key=' . $text);
		}

		if ($info == '暂不支持此彩种') {
			$info = '您好，我是[***]微信小助手，请问有什么需要帮助的吗？回复H获取帮助菜单！';
		}
