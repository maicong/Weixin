...
class Wechat
{
	...
	*/
	...
	/**
	 * 小黄鸡自动回复
	 * Examle: $obj->xiaohuangji('您好您好')->reply();
	 * @param string $text
	 */
	public function xiaohuangji($text = '')
	{
		$text = preg_replace('/\s/', '', $text);

	
		$info = $this->http_post('http://www.niurenqushi.com/app/simsimi/ajax.aspx','txt='.$text);
		...
		if ($info == $text) {
			$info = '你说的话好有道理，我竟无言以对。';
		}

		$info = preg_replace('/\b(https?|ftp|file):\/\/[-A-Z0-9+&@#\/%?=~_|$!:,.;]*[A-Z0-9+&@#\/%=~_|$]/iu', '', $info);
